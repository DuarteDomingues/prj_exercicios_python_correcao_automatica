...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		var = random.sample(string.ascii_lowercase, 4)
		self.__func = var[0]

		numbers_list = random.choice(range(100, 1000))
		self.__numbers = numbers_list

		list_strings = ["f", "x", "g", "y","3"]


		self.add_changable_strings(list_strings)

		change_all_occurrences(list_strings[0], var[0])
		change_all_occurrences(list_strings[1], var[1])
		change_all_occurrences(list_strings[2], var[2])
		change_all_occurrences(list_strings[3], var[3])
		change_token_all_occurrences(list_strings[4], str(numbers_list))

		self.__output = [numbers_list+2,numbers_list+2]

		self.__output_errado = [numbers_list+1, numbers_list+2]
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		list_strings = [r"\verb+output+", r"\verb+outputErrado+"]

		self.add_changable_strings(list_strings)

		change_all_occurrences(list_strings[0], str(self.__output))
		change_all_occurrences(list_strings[1], str(self.__output_errado))
