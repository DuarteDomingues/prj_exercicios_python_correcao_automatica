...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		x_var = random.choice(string.ascii_letters)

		list_nums = random.sample(range(5,30),5)


		list_strings = ["x","1","2","10","5","1000"]

		self.add_changable_strings(list_strings)

		change_token_all_occurrences(list_strings[0], x_var.lower())
		change_all_occurrences(list_strings[1], str(list_nums[0]))
		change_all_occurrences(list_strings[2], str(list_nums[1]))
		change_all_occurrences(list_strings[3], str(list_nums[2]))
		change_all_occurrences(list_strings[4], str(list_nums[3]))
		change_all_occurrences(list_strings[5], str(list_nums[4]))
