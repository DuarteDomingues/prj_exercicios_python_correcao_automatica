...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		strgs = ["camelo", "pato", "pinguim", "toupeira", "vaca", "papagaio", "porco", "lobo", "rapoza", "zebra", "macaco", "gorila", "koala", "urso", "rinoceronte", "lebra", "tartaruga"]

		[str_cao,str_gato] = random.sample(strgs,2)

		[str_x,str_y] = random.sample(string.ascii_lowercase, 2)

		list_strings = ["cao","gato","x","y"]

		self.add_changable_strings(list_strings)

		change_token_all_occurrences("x", str_x)
		change_token_all_occurrences("y", str_y)
		change_all_occurrences("cao", str_cao)
		change_all_occurrences("gato", str_gato)
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		list_strings = [r'\verb+NameError+']

		self.add_changable_strings(list_strings)

		list_errors = ['NameError', 'IndentationError','TabError','TypeError', 'IndexError']

		chosen_error = random.choice(list_errors)

		change_all_occurrences(r'\verb+NameError+', r'\verb+' +chosen_error + '+')
