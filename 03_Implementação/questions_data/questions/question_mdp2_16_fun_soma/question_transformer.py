...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		numbers_list = random.sample(range(1,25), 4)
		self.__numbers = numbers_list

		list_strings = ["f", "x", "10", "5", "1.5", "0"]
		...
		change_token_all_occurrences(list_strings[4], str(numbers_list[2]))
		change_token_all_occurrences(list_strings[5], str(numbers_list[3]))
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		for i in range(len(self.__numbers)):
			result_true+= str(self.__numbers[i]*2)+r'\newline'+"\n"
			result_false+= str(self.__numbers[i])+r'\newline'+"\n"
