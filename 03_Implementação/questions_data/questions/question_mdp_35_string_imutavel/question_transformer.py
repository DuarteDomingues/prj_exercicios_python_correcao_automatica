...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		first_str = ''.join(random.choices(string.ascii_lowercase, k=5))

		second_str = ''.join(random.choices(string.ascii_lowercase, k=5))

		variable = random.choice(string.ascii_lowercase)

		list_strings = ["hello world!", "x", "H", "W"]

		self.add_changable_strings(list_strings)

		change_all_occurrences(list_strings[0], first_str+" "+second_str+"!")
		change_all_occurrences(list_strings[1], variable)
		change_all_occurrences(list_strings[2], first_str[0].upper())
		change_all_occurrences(list_strings[3], second_str[0].upper())
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		errors_type = ["GeneratorExit", "KeyError", "SyntaxError", "ReferenceError", "RuntimeError", "UnicodeError", "NameError"]
