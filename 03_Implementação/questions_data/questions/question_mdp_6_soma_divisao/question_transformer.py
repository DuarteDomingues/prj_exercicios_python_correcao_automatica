...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		list_numbers_to_change = random.sample(range(1, 50), 4)

		list_strings = ["2","3","4","5"]

		self.add_changable_strings(list_strings)

		change_all_occurrences("2",str(list_numbers_to_change[0]))
		change_all_occurrences("3",str(list_numbers_to_change[1]))
		change_all_occurrences("4",str(list_numbers_to_change[2]))
		change_all_occurrences("5",str(list_numbers_to_change[3]))
