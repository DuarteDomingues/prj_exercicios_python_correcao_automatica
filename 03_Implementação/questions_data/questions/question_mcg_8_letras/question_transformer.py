...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		str_isel = ""
		strgs = ["dudu", "daisy", "siwi", "lp", "xpto", "kappa", "kek", "pog", "nono", "clap", "baby yoda", "pizza", "boy", "png", "dom", "baba", "k2"]
		rd_strg = ''.join(random.choice(string.ascii_lowercase) for i in range(4))

		if random.randint(0,10) > 4:
			str_isel = (random.choice(strgs))
		...
		change_all_occurrences("isel", str_isel)

		self.__isel = str_isel
		self.__i = str_isel[0]
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		list_strings = [r'\verb+isel+',r'\verb+i+']

		self.add_changable_strings(list_strings)


		change_all_occurrences(r'\verb+isel+', r'\verb+' +self.__isel + '+')
		change_all_occurrences(r'\verb+i+', r'\verb+' +self.__i + '+')
