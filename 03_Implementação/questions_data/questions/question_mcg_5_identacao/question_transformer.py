...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		[class_x] = random.choice(string.ascii_uppercase)

		[var_a, var_x, var_y] = random.sample(string.ascii_lowercase, 3)

		list_strings = ["A","a","x","y"]

		self.add_changable_strings(list_strings)


		change_all_occurrences("A", class_x[0])
		change_token_all_occurrences("a", var_a[0])
		change_token_all_occurrences("x", var_x[0])
		change_all_occurrences("y", var_y[0])
