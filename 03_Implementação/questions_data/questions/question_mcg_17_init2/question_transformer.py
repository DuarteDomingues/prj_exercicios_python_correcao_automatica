...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		[str_x, str_y, str_z] = random.sample(string.ascii_lowercase, 3)
		self.__z = str_z

		numbers = random.sample(range(1, 100), 2)

		list_strings = ["Z", "x", "y", "z", "10", "20"]

		self.add_changable_strings(list_strings)

		change_token_all_occurrences(list_strings[0], str_z.upper())
		change_token_all_occurrences(list_strings[1], str_x)
		change_token_all_occurrences(list_strings[2], str_y)
		change_token_all_occurrences(list_strings[3], str_z)
		change_token_all_occurrences(list_strings[4], str(numbers[0]))
		change_token_all_occurrences(list_strings[5], str(numbers[1]))
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		list_strings = [r'\verb+z+']

		self.add_changable_strings(list_strings)

		change_all_occurrences(r'\verb+z+', r'\verb+' +self.__z + '+')
