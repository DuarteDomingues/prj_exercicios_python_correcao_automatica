...
class QuestionTransformer(GenerateRandomVersion):
	...
	def create_python_program(self, python_files_list, version):
		...
		change_all_occurrences(list_strings[2], letters[2])
		...
		change_token_all_occurrences(list_strings[6], str(numbers_list[3]))
	...
	def create_latex_files(self, general_latex_files, version, output):
		...
		list_strings = [r'\verb+lel+', r'\verb+lel2+', r'\verb+lel3+', r'\verb+lel4+']

		not_error = r'\verb+não dá+'

		error = r'\verb+dá+'

		self.add_changable_strings(list_strings)

		change_all_occurrences(list_strings[0], not_error)
		change_all_occurrences(list_strings[1], error)
		change_all_occurrences(list_strings[2], error)
		change_all_occurrences(list_strings[3], not_error)
